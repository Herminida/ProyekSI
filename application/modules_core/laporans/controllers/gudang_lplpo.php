<?php 
class Gudang_lplpo extends CI_Controller {

    public function __construct() {
        parent::__construct();
//        $this->load->model('gudang_penerimaan_model');
        $this->load->model('gudang_lplpo_model');
...
//        error_reporting(E_ALL ^ (E_NOTICE));
        if($this->input->post('bulan_laporan')==''){
            ...
            $data['tanggallaporan1']=date('Y-m-d');
            $data['tanggallaporan2']=date('Y-m-d');
        }else{
            ...
            $data['tanggallaporan1']=$this->input->post('tahun_laporan1').'-'.$this->input->post('bulan_laporan1').'-'.$this->input->post('tanggal_laporan1');
            $data['tanggallaporan2']=$this->input->post('tahun_laporan2').'-'.$this->input->post('bulan_laporan2').'-'.$this->input->post('tanggal_laporan2');
            
            
        }
        
        $data['obats']=$this->farmasi_obats_model->Get_Farmasi_Obats();
        $data['lasts']=$this->gudang_lplpo_model->getlplpoBydate($tanggal);
        $data['pengeluarans']=$this->gudang_lplpo_model->getPengeluaranBydate($tanggal1,$tanggal2);
        $data['penerimaans']=$this->gudang_lplpo_model->getPenerimaanBydate($tanggal1,$tanggal2);
        $data['stokawals']=$this->gudang_lplpo_model->getStokawalBydate($tanggalstok);
        $this->template->load('template','gudang_lplpo/index',$data);
    }
    
    public function generateLplpo(){
        $tanggal=$this->input->post('tanggal_generate');
        $ceklplpos=$this->gudang_lplpo_model->getlplpoBydate($tanggal);
        $ceklplpo = $ceklplpos->num_rows;
        if($ceklplpo > 0){
            redirect('laporans/farmasi_lplpo');
        }else{
            ...
            for($i=1;$i<=$katalogobat;$i++){
                $fields=array(
                    ...
                    'unit_kerja_id'=>$this->session->userdata("id_unit_kerja")
                );
                $this->gudang_lplpo_model->generateLplpo($fields);
            }
            redirect('laporans/gudang_lplpo');
...
//        $data['stokawals']=$this->apotek_lplpo_model->getStokawalBydate($tanggalstok);
        $this->load->view('gudang_lplpo/cetaklplpo',$data);
    }
