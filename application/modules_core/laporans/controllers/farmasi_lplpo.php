<?php 
class Farmasi_lplpo extends CI_Controller {

    public function __construct() {
        parent::__construct();
//        $this->load->model('farmasi_penerimaan_model');
        $this->load->model('farmasi_lplpo_model');
...
//        error_reporting(E_ALL ^ (E_NOTICE));
        ...
        $data['lasts']=$this->farmasi_lplpo_model->getlplpoBydate($tanggal);
        $data['pengeluarans']=$this->farmasi_lplpo_model->getPengeluaranBydate($tanggal1,$tanggal2);
        $data['penerimaans']=$this->farmasi_lplpo_model->getPenerimaanBydate($tanggal1,$tanggal2);
        $data['stokawals']=$this->farmasi_lplpo_model->getStokawalBydate($tanggalstok);
        $this->template->load('template','farmasi_lplpo/index',$data);
    }
    
    public function generateLplpo(){
        $tanggal=$this->input->post('tanggal_generate');
        $ceklplpos=$this->farmasi_lplpo_model->getlplpoBydate($tanggal);
        ...
        if($ceklplpo > 0){
            ...
            for($i=1;$i<=$katalogobat;$i++){
                ...
                $this->farmasi_lplpo_model->generateLplpo($fields);
            }
            redirect('laporans/farmasi_lplpo');
...
//        $data['stokawals']=$this->apotek_lplpo_model->getStokawalBydate($tanggalstok);
        $this->load->view('farmasi_lplpo/cetaklplpo',$data);
    }
