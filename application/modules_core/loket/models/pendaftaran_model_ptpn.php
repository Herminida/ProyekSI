<?php
 class pendaftaran_model_ptpn extends Ci_model{
 	function add($insert){
 	...
 	function get_data(){
 		return $this->db->query("select * from tbl_pasien where nama_ptpn !=''");
 	}
 	...
 	function search($kata_kunci){
 		return $this->db->query("select * from tbl_pasien where (nama_lengkap like '%$kata_kunci%' or nama_panggilan like '%$kata_kunci%' or no_identitas like '%$kata_kunci%' or no_rm like '%$kata_kunci%') and nama_ptpn !=''");
 	}
