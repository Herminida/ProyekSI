  ...
  <head>
    ...
    <script>
          ...
          count += 1;
            ...
            $("#jumlah_"+count).live('click',function(){ //pembuka validasi obat
              var obat=$("#obat_id_"+count).val();
                        $.ajax({
                            type:"POST",
                            url: "<?php echo base_url(); ?>farmasi/farmasi_pengeluaran/falidasiobat",
                            data: "obat="+obat ,
        ...
        $("#jumlah_"+count).live('keyup',function(){
            ...
            $("#simpan").show();
            var obat=$("#obat_id_"+count).val();
            var jumlah=$("#jumlah_"+count).val();
            var bulan=$('#bln').val();
            var tahun=$('#thn').val();
           
                        $.ajax({
                            type:"POST",
                            url :"<?php echo base_url();?>farmasi/farmasi_pengeluaran/falidasijumlahobat",
            ...
            $("#obat_id_"+count).live('focus',function(){ // pembuka auto complate
               $("#obat_id_"+count).autocomplete({
                ...
                function(req, add){
                    $.ajax({
                        url: "<?php echo base_url(); ?>farmasi/farmasi_pengeluaran/lookup",
                        dataType: 'json',
                        type: 'POST',
                        data: req,
                        success:    
    ...
    $(".btn2").live('click',function() {
                ...
                var obat=$("#obat_id_"+nomor).val();
                        $.ajax({
                            type:"POST",
                            url: "<?php echo base_url(); ?>farmasi/farmasi_pengeluaran/falidasiobatbatal",
                            data: "obat="+obat ,
                            success : function (data) {
                                $("#tampilkandata").html(data);
                            }
...
<body>
  <div id="real">
    ...
    <?php echo form_open('farmasi/farmasi_pengeluaran/save_add'); ?>
    <?php foreach ($farmasi_pengeluaran_header->result_array() as $data) { ?>
    <table class="table">
        ...
        <tbody>
          ...
          <input type="hidden" name="tanggal_keluar" value="<?php echo $data['tanggal_keluar'] ?>" >
          <input type="hidden" name="tujuan" value="<?php echo $data['tujuan2'] ?>" >
          <input type="hidden" name="unit_kerja_id" value="<?php echo $this->session->userdata("id_unit_kerja"); ?>" >
          <input type="hidden" id="thn" value="<?php echo $data['thn']; ?>">
          <input type="hidden" id="bln" value="<?php echo $data['bln']; ?>">
          <tr>
                <td>No Nota</td>
                <td>:</td>
                <td>
