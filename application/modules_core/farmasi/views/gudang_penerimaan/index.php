        ...
        $("#jumlah_"+count).live('keyup',function(){
            ...
            $("#obat_id_"+count).live('focus',function(){ // pembuka auto complate
               $("#obat_id_"+count).autocomplete({
                ...
                function(req, add){
                    $.ajax({
                        url: "<?php echo base_url(); ?>farmasi/gudang_penerimaan/lookup",
                        dataType: 'json',
                        type: 'POST',
                        data: req,
                        success:    
...
<body>
	<div id="real">
		<div id="statusdonor">
			<?php
				if ($id_gudang_header->num_rows() >0) {
					foreach ($id_gudang_header->result() as $data) {
						$gudangpenerimaanid = $data->id+1;
					}

				}
				else {

					$gudangpenerimaanid = date ('Ymd').'001';

				}
			?>
			<?php echo form_open('farmasi/gudang_penerimaan/save'); ?>
			<table class="table table-striped">
				<thead>
					<tr>
						<th colspan="4">::.Gudang Penerimaan</th>
					</tr>
				</thead>
				<input type="hidden" name="id"  value="<?php echo $gudangpenerimaanid ?>" >
          		<input type="hidden" name="terima_id" id="terima" value="<?php echo $gudangpenerimaanid ?>" >
            ...
            <td>
              	<select style="width:250px; height:25px; font-size:12px;" name="supplier_id" id="supplier_id">
        			...
        			<?php
            			...
            			if ($id_supplier==$value['id_supplier']){
        			?>
