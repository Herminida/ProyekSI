...
class Mutasi_pengeluaran_model extends CI_Model {
	...
	public function Ambil_Gudang_Pengeluaran () {
		...
		return $this->db->query("select * from mutasi_pengeluaran a left join farmasi_obats b ON a.obat_id = b.id left join unit_kerja c ON a.unit_kerja_id = c.id_unit_kerja ");
	}
	...
	public function getpengeluaranid () {
		...
		$this->db->select('*');
        $this->db->from("mutasi_pengeluaran_header");
	...
	public function Get_Pengeluaran_header ($id) {
		...
		return $this->db->query("select a.*,DATE_FORMAT(a.tanggal_keluar,'%d %b %Y') as tanggal,b.nama_unit_kerja from mutasi_pengeluaran_header a 
		join  unit_kerja b on a.unit_kerja_id = b.id_unit_kerja 
			where a.id='$id' ");
 ...
 ");
	...
	public function Get_Pengeluaran ($tgl,$dari) {

		return $this->db->query("select a.*,DATE_FORMAT(a.tanggal_keluar,'%d %b %Y') as tanggal, b.nama_unit_kerja from mutasi_pengeluaran_header a 
			join unit_kerja b on a.unit_kerja_id=b.id_unit_kerja 
			where a.tanggal_keluar='$tgl' and a.unit_kerja_id='$dari' order by a.id desc");
	...
	public function seacrh ($tgl,$bln,$thn,$dari) {
		...
		return $this->db->query("select a.*,DATE_FORMAT(a.tanggal_keluar,'%d %b %Y') as tanggal,DAY(tanggal_keluar) as tgl2,MONTH(tanggal_keluar) as bulan2,YEAR(tanggal_keluar) as thn2,b.nama_unit_kerja from mutasi_pengeluaran_header a
		join unit_kerja b on a.unit_kerja_id = b.id_unit_kerja  
where DAY(tanggal_keluar) ='$tgl' and MONTH(tanggal_keluar) ='$bln' and YEAR(tanggal_keluar) ='$thn' and a.unit_kerja_id='$dari' order by a.id desc");
	...
	public function cekdoubleobat ($obat_id,$keluar_id) {
		return $this->db->query("select * from mutasi_pengeluaran_item where obat_id='$obat_id' and keluar_id='$keluar_id' ");
	}
