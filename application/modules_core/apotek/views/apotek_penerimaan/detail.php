...
<head>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      <?php 
        $n = 1;
        if($detail_apotek_penerimaan2->num_rows()>0)
        {
          foreach($detail_apotek_penerimaan2->result_array() as $data)
          {
          ...
          echo "
            ...
            $(\"#tampilketeranganr".$n."\").hide();
              $(\"#tampiljumlah".$n."\").hide();
              $(\"#tampiljumlahr".$n."\").hide();
         ...
         $(\"#simpan".$n."\").click(function() {
              var jumlah = $(\"#notar".$n."\").val();
              var keterangan = $(\"#keterangan2".$n."\").val();
              var id2 = $(\"#id".$n."\").val();

               $.ajax({
                            type:\"POST\",
                            url :\"".base_url()."apotek/apotek_penerimaan/updateid\",
...
<body>
  <div id="real">
    <div id="statusdonor">
      <?php echo form_open('apotek/apotek_penerimaan/cek_list');?>
      <?php foreach ($apotek_penerimaan_header2->result_array() as $h) { ?>
...
<table class="table table-striped">
        ...
        <tr>
          ...
          <td>
            <label id="jumlahr<?php echo $no ; ?>"><?php echo $data['jumlah_gudang']; ?></label>
            <input type="text" class="span1" id="notar<?php echo $no;?>" value="<?php
                    echo $data['jumlah_gudang']; ?>"> <div id="tampiljumlahr<?php echo $no ; ?>">coba</div>
          </td>
          <td>
          ...
          <td>
            ...
            $id = $h['id'];
            $cek = "sudah";
            $query = $this->apotek_penerimaan_model->sudahada($id,$cek);
          ...
          echo "&#x2713";
            ...
            <a href="<?php echo base_url();?>apotek/apotek_penerimaan/delete_item/<?php echo $data['keluar_id'];?>/<?php echo $data['id'];?>" class="btn btn-small" onclick="return confirm('Apakah Anda Yakin Ingin Mengahapus <?php echo $data['nama_obat']; ?> ?')" id="delete<?php echo $no ; ?>"><i class="icon icon-remove"></i></a>
          <?php
          }
            ?>
          </td>
      ...
      <a href="<?php echo base_url();?>apotek/apotek_penerimaan/view_penerimaan" class="btn btn-small">Kembali</a>
      <a class="btn btn-small" onclick="javascript:window.open('<?php echo base_url()?>apotek/apotek_penerimaan/cetak/<?php echo $h['id']; ?>','mywin','left=20,top=10,width=590,height=600,toolbar=0,resizable=0')"><i class="icon icon-print"></i>Cetak</a>
