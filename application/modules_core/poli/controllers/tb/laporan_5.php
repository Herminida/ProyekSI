...
class laporan_5 extends CI_Controller {
    ...
    function index(){
        $this->template->load('templateTB','tb/v_laporan_5');
    }
    ...
    function _generate($mode,$tgl_awal,$tgl_akhir){
        ...
        if(isset($view['data']) and $view['data']){
            $this->load->view('tb/generate_laporan_5',$view);
        }
    }
     ...
     function _generateedit($mode,$tgl_awal,$tgl_akhir){
        ...
        $view['tgl_akhir'] = $tgl_akhir;
            $this->load->view('tb/generate_laporan_5_edit',$view);
    }
