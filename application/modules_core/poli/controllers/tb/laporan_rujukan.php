...
class laporan_rujukan extends CI_Controller {
    ...
    function index(){
        $this->template->load('templateTB','tb/v_laporan_rujukan');
    }
    ...
    function _generate($mode,$no_rm_tb){
        ...
        if(isset($view['kpp']['id'])){
            $view['dosis'] = $this->mtb_kpp->get_dosis($view['kpp']['id']);
            $view['data'] = $this->mtb_rujukan->detail_by_kpp($view['kpp']['id']);
            $view['permohonan'] = $this->mtb_permohonan->get_data_by_rm_tb('abc',$no_rm_tb);
            if(isset($view['permohonan']['id_permohonan_lab'])){
                $view['hasil'] = $this->mtb_lab->detail_by_permohonan($view['permohonan']['id_permohonan_lab']);
            }
        }
        // print_r($view);
        if(isset($view['data']) and $view['data']){
            $this->load->view('tb/generate_laporan_rujukan',$view);
        }
    }
