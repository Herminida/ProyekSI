    <div class="span9"> <!-- content span -->
      <input type="hidden" id="id_poli" name="id_poli" value='10'>
      <ul class="nav nav-tabs" id="tab-link">
    <li class="active"><a href="#" onclick="return false">Lansia</a></li>
...
function getSisaAntrian(){
      $.ajax({
            url: '<?php echo site_url('poli/polilansia/ajaxsisaantrian')?>',
           // data: data,
            method:'post',
            dataType: 'json',
  ...
  jQuery(document).ready(function(){
      ...
      setInterval("getSisaAntrian()",60000);
        ...
        if(err){
          ...
          console.log(data);
          $.ajax({
            url: '<?php echo site_url('poli/'.'polilansia/simpan')?>',
            data: data,
            method:'post',
            dataType: 'json',
            success: function(obj){
    ...
    var purl = '<?php echo site_url("poli/polilansia/ajaxpemeriksaan");?>'+'/'+sData.idpemeriksaan;
