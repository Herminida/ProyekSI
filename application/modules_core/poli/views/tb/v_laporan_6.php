<div class="span9">
    <ul class="nav nav-tabs" id="tab-link">
        <li class="active"><a href="#" onclick="return false">6. Laporan Triwulan Hasil Pemeriksaan Dahak Mikroskopis Akhir Tahap Intensif</a></li>
    </ul>
...
$(function(){
    $('#btnTampilkan').click(function(){
        $('#preview').load('<?php echo site_url('poli/'.'tb/laporan_6/preview_')?>/'+$('#tgl_awal').val()+'/'+$('#tgl_akhir').val(),function(response,status,xhr){
        ...
        $('#btnPrint').attr('href','<?php echo site_url('poli/'.'tb/laporan_6/print_')?>/'+$('#tgl_awal').val()+'/'+$('#tgl_akhir').val());
        $('#btnExcel').attr('href','<?php echo site_url('poli/'.'tb/laporan_6/excel_')?>/'+$('#tgl_awal').val()+'/'+$('#tgl_akhir').val());
    });
    $('#btnPrint').hide();
