<div class="span9">
    <ul class="nav nav-tabs" id="tab-link">
        <li class="active"><a href="#" onclick="return false">Hasil Pemeriksaan Ulang Sputum BTA</a></li>
    </ul>
...
$(function(){
    $('#btnTampilkan').click(function(){
        $('#preview').load('<?php echo site_url('poli/'.'tb/laporan_ulang/preview_')?>/'+$('#dari').val()+'/'+$('#sampai').val(),function(response,status,xhr){
        ...
        $('#btnPrint').attr('href','<?php echo site_url('poli/'.'tb/laporan_ulang/print_')?>/'+$('#dari').val()+'/'+$('#sampai').val());
        $('#btnExcel').attr('href','<?php echo site_url('poli/'.'tb/laporan_ulang/excel_')?>/'+$('#dari').val()+'/'+$('#sampai').val());
    });
    $('#btnPrint').hide();
