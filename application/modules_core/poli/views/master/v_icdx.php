 ...
 <!--/span9-->
   ...
   function isikelompok(){
    ...
    function isisubkelompok(){
      var items="";
      var purl = '<?php echo site_url("poli/master/icdx/ajaxSubKelompok");?>'+'/'+$('#kode_kelompok').val();
       $.getJSON(purl,function(kdatas){
       if (jQuery.isEmptyObject(kdatas)){
      }else{
        for(var i in kdatas){
   ...
   function hapus(){
    ...
    data={ id:kelicdData.id};
     $.ajax({
            url: "<?php echo site_url('poli/'.'master/icdx/hapuspenyakit')?>",
            data: data,
            method:'post',
            dataType: 'json',
...
function GridRowCount() {
  ...
  jQuery(document).ready(function(){
      ...
      "bScrollCollapse": false, 
        "bProcessing": true,
        "bServerSide": true,
        "sAjaxSource": "<?php echo site_url('poli/'.'master/icdx/getlistPenyakit')?>",
        "aoColumns": [
               { "mData": "nama_kelompok" }
              ,{ "mData": "kode_penyakit" }
              ,{ "mData": "nama_penyakit" }
      ...
      $('#frmIcdx').submit(function(){
        ...
        if(err){
          ...
          $.ajax({
            url: '<?php echo site_url('poli/'.'master/icdx/simpanPenyakit')?>',
            data: data,
            method:'post',
            dataType: 'json',
            success: function(obj){
